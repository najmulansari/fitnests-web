...
import {
  Plus, X, Tag, ChevronUp, ChevronDown,
...
function ImageUploadField({ imageUrl, onImageUrl }) {
  ...
  // Sync if parent clears imageUrl
  useEffect(() => {
...
function AddCategoryModal({ onClose, onSave, createdBy }) {
  ...
  return (
    <ModalShell title="Add New Category" subtitle="Create a new listing category" onClose={onClose}>
...
function ViewCategoryModal({ cat, onClose, onEdit }) {
  return (
    <ModalShell title="Category Details" subtitle="View category information" onClose={onClose}>
      <div className="flex flex-col gap-4">
        {/* Image */}
        {cat.imageUrl ? (
          <div className="rounded-xl overflow-hidden border border-gray-100">
            <img src={cat.imageUrl} alt={cat.name} className="w-full h-44 object-cover" />
          </div>
        ) : (
        ...
        <div className="grid grid-cols-2 gap-3 text-sm">
          <Detail label="Name" value={cat.name} full />
          <Detail label="Created By" value={cat.createdBy} />
          <Detail label="Updated By" value={cat.updatedBy} />
          <Detail label="Created Date" value={formatDate(cat.createdDate)} />
          <Detail label="Updated Date" value={formatDate(cat.updatedDate)} />
        </div>
...
function EditCategoryModal({ cat, onClose, onSave, updatedBy }) {
  ...
  return (
    <ModalShell title="Edit Category" subtitle={`Editing "${cat.name}"`} onClose={onClose}>
      <form onSubmit={handleSubmit} noValidate className="flex flex-col gap-4">
        <div>
          <label className="block text-sm font-medium text-gray-700 mb-1.5">
            Category Name <span className="text-red-500">*</span>
...
function DeleteCategoryModal({ cat, onClose, onDeleted }) {
  const [submitting, setSubmitting] = useState(false);
  const [error, setError]           = useState("");
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center px-4">
      ...
      <div className="relative bg-white rounded-xl shadow-2xl w-full max-w-sm p-6">
        ...
        <h4 className="text-center text-base font-bold text-gray-900 mb-1">Delete Category</h4>
  ...
  useEffect(() => {
    fetch("/api/categories")
      ...
      .then((data) => {
        ...
        else setFetchError(data.error || "Failed to load categories.");
      })
    ...
    <>
                  ...
                  {sorted.length === 0 ? (
                    <tr>
                      <td colSpan={6} className="px-5 py-14 text-center">
                        ...
                        <p className="text-xs text-gray-400 mt-1">Click &ldquo;Add New Category&rdquo; to get started</p>
                      </td>
                    </tr>
                  ) : sorted.map((cat, idx) => (
                    <tr key={cat.id}
                      ...
                      <td className="px-5 py-3.5 text-gray-600 whitespace-nowrap">{formatDate(cat.createdDate)}</td>
                      ...
                      <td className="px-5 py-3.5 text-gray-600 whitespace-nowrap">{formatDate(cat.updatedDate)}</td>
                      ...
                      <td className="px-5 py-3.5">
                        <div className="flex items-center justify-end gap-0.5">
                          <button onClick={() => setViewTarget(cat)}
      ...
      {showAdd && (
        <AddCategoryModal
          ...
          onSave={(saved) => { setCategories((p) => [...p, mapCategory(saved)]); setShowAdd(false); }}
        />
      )}
      ...
      {editTarget && (
        <EditCategoryModal
          ...
          onSave={(saved) => {
            setCategories((p) => p.map((c) => (c.id === saved.id ? mapCategory(saved) : c)));
            setEditTarget(null);
          }}
        />
      )}
      ...
      {deleteTarget && (
        <DeleteCategoryModal
          ...
          onDeleted={(id) => { setCategories((p) => p.filter((c) => c.id !== id)); setDeleteTarget(null); }}
        />
      )}
    </>
  );
